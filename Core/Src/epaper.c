...
void epd_res_set() {
	...
	HAL_GPIO_WritePin(eink_res_GPIO_Port, eink_res_Pin, 1);
}

void epd_res_reset() {
	//gpio_bits_reset(GPIOA, GPIO_PINS_8);
	HAL_GPIO_WritePin(eink_res_GPIO_Port, eink_res_Pin, 0);
}

void epd_dc_set() {
	//gpio_bits_set(GPIOB, GPIO_PINS_14);
	HAL_GPIO_WritePin(eink_dc_GPIO_Port, eink_dc_Pin, 1);
}

void epd_dc_reset() {
	//gpio_bits_reset(GPIOB, GPIO_PINS_14);
	HAL_GPIO_WritePin(eink_dc_GPIO_Port, eink_dc_Pin, 0);
}

void epd_cs_set() {
	//gpio_bits_set(GPIOB, GPIO_PINS_12);
	HAL_GPIO_WritePin(eink_cs_GPIO_Port, eink_cs_Pin, 1);
}

void epd_cs_reset() {
	//gpio_bits_reset(GPIOB, GPIO_PINS_12);
	HAL_GPIO_WritePin(eink_cs_GPIO_Port, eink_cs_Pin, 0);
}

uint8_t epd_is_busy() {
	//return gpio_input_data_bit_read(GPIOA, GPIO_PINS_9) == RESET ? 0 : 1;
	return HAL_GPIO_ReadPin(eink_busy_GPIO_Port, eink_busy_Pin);
}
